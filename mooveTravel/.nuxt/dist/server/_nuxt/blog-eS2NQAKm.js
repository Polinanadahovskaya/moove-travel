  ...
  async setup(__props) {
    ...
    const filteredArticles = computed(() => {
      return articles.value.filter((art) => {
        ...
        if (selectedType.value.length && !(art.article_tags || []).some((tag) => selectedType.value.includes(tag?.name))) match = false;
...
//# sourceMappingURL=blog-eS2NQAKm.js.map
